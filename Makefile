...
algorithm: join filter

ingest: create_db ingest
...
load_news: config/load_news.yaml
	python3 run.py load_news --config=config/load_news.yaml --output=${daily_news}

load_wiki: config/load_wiki.yaml ${daily_news}
	python3 run.py load_wiki --config=config/load_wiki.yaml --input=${daily_news} --output=${daily_wiki}

join: ${daily_news} ${daily_wiki}
	python3 run.py join --input1=${daily_news} --input2=${daily_wiki} --output=${daily_joined} --s3_path=${s3_bucket}

filter: config/algorithm.yaml ${daily_joined}
